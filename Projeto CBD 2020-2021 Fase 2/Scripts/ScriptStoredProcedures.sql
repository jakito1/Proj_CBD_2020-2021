...
-- exec InscreverAlunoDisciplina 1,1;
...
create or alter procedure AtualizarNota
	...
	begin 
		...
		if(@Period = 'p1')
		begin 
		update Grades set P1 = @NewGrade where GStudentID = @StudentNumber and GSubjectID = @SubjectID;
		end
		else if(@Period = 'p2')
		begin 
		update Grades set P2 = @NewGrade where GStudentID = @StudentNumber and GSubjectID = @SubjectID;
		end
		else if(@Period = 'p3')
		begin 
		update Grades set P3 = @NewGrade where GStudentID = @StudentNumber and GSubjectID = @SubjectID;
		end
		else
		begin 
...
create or alter procedure CriarDisciplina
	@Name_en varchar(10), @Description_en varchar(100), @Name_pt varchar(10), @Description_pt varchar(100), @AreaID int
	as
	if(@Name_en is null or @Description_en is null or @Name_pt is null or @Description_pt is null or @AreaID is null)
	begin
		insert into Errors values(SYSTEM_USER, CURRENT_TIMESTAMP, 'ERROR: Invalid Data (cannot be null) | Dados inválidos (Não podem ser null)')
		raiserror(15600, 16,1, 'ERROR: Invalid Data (cannot be null) | Dados inválidos (Não podem ser null)')
	end
	...
	begin
		insert into Subjects(SName_en, SDescription_en, SName_pt, SDescription_pt, SAreaID) select @Name_en, @Description_en, @Name_pt, @Description_pt, @AreaID 
	end
go
